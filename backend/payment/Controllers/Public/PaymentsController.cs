...
public class PaymentsController : PaymentControllerBase
{
    ...
    private string RedirectFrontendAddress(object result)
    {
        ...
        var queryString = Minisource.Common.Extensions.StringExtensions.ToQueryString(result).ToLower();
    ...
    {
        var result = await _paymentQueryService.GetPaymentsAsync(status, from, to, page, pageSize, cancellationToken);
        return Ok(result);
    }
    ...
    public async Task<ActionResult<List<PaymentLogDto>>> GetPaymentLogs(
        ...
        CancellationToken cancellationToken)
    {
        var logs = await _paymentQueryService.GetPaymentLogsAsync(id, cancellationToken);
        return Ok(logs);
    }
}
